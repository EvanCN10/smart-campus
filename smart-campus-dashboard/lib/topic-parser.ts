...
export function parseMqttMessage(topic: string, payload: Buffer): void {
  ...
  // Response topic: campus/rr/room-snapshot/response/{clientId}
  if (category === "rr" && segments.length === 5) {
    const feature = segments[2];
    const kind = segments[3];
    if (feature === "room-snapshot" && kind === "response") {
      const data = parsed as RoomSnapshotResponse;
      store.setRoomSnapshotSuccess(data);
      return;
    }
  }
