...
client.on("connect", () => {
  ...
  client.subscribe("campus/#", { qos: 1 }, (err) => {
    if (!err) {
      console.log("[Logger Subscriber] Subscribed to campus/# with QoS 1");
    } else {
...
client.on("message", (topic, message, packet) => {
  ...
  if (topic.includes("alerts") || topic.includes("announcements") || topic.includes("system")) {
    console.log(`[LOGGER] Topic: ${topic} | QoS: ${packet.qos} | Retained: ${packet.retain}`);
